function outfile_=stranger_elim(str_file_path,str_file,elim_list)
%function outfile_=stranger_elim(str_file_path,str_file,elim_list)
%Translates the stranger file and also eliminates trials given in elim_list.
%ELIMINATE a LIST of TRIALS
%CALLS: None
%CALLER: Command line
%Example: O=stranger('c:\fecfef\','f_fef000.str',TriallistToEliminate)
%Author: %chenchal.subraveti@vanderbilt.edu
global Translation
if(nargin==2)
   elim_list=0;
else
   Translation='Translating Stranger file :::with selected trials eliminated'
end
...
if(elim_list)
   save(outfile_,'elim_list','-append')
end
...
for n=1:Neurons
   ...
   for k=1:TotalTrials
      if(~length(find(elim_list==k)))
         ...
         temp=[];
      else %Trial No foud in eliminate trials list
         temp=0;
         eval(strcat(M_SpikeID(n,:),'(k,1:length(temp))=','temp;'));
      end
   end %
